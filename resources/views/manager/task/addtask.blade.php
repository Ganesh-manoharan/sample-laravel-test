...
<div class="content-wrapper">
  ...
  <section class="content">
    <!-- container-fluid -->
    <form id="regForm" name="regForm" action="{{ route('savetaskdetails') }}" method="POST" enctype="multipart/form-data">
      {{ csrf_field() }}
      <input type="hidden" value="{{$formType}}" name="formType">
      <input type="hidden" name="parent_task" id="parent_task" value="{{request()->task_id}}" hidden>
      <input type="hidden" name="mode" id="mode" value="{{ request()->task_id?'parent':'add'}}">
      <div class="container-fluid addtask-section">
        <!-- tab  -->
        <div class="tab wizard-tab">
          <div class="task-title">
            <h3>{{ ucfirst($formType)}} Creation @if(isset($formFields['two'])) Step One @endif</h3>
          </div>
          <div class="create-task-form">
            <div class="row align-items-start">
              ...
              @if($details->fieldType=='number')
                <input @if($details->is_requried) required="requried" @endif class="form-control number" type="number" id="task_{{$details->code}}" placeholder="{{$details->placeholder}}" name="formField[{{$details->id}}]" autocomplete="off" />
              @elseif($details->fieldType=='long_text')
                <textarea @if($details->is_requried) required="requried" @endif class="form-control" placeholder="{{$details->placeholder}}" id="task_{{$details->code}}" name="formField[{{$details->id}}]"></textarea>
              @elseif($details->fieldType=='date')
                <input @if($details->is_requried) required="requried" @endif type="text" name="formField[{{$details->id}}]" class="form-control datePicker" id="task_{{$details->code}}" data-value="task-{{$details->code}}" placeholder="{{$details->placeholder}}" autocomplete="off">
              @elseif(in_array($details->fieldType,array('dropdown_value','select2')))
              
                  @php
                  $selectedOption=$taskFieldDetails[$details->code]??'';
                  ...
                  @endphp
              <select @if($details->is_requried) required="requried" @endif class="form-control select2" id="task_{{$details->code}}" id="task_{{$details->code}}" data-selectedID="{{$selectedText}}" @if($details->fieldType=='select2') multiple="multiple" name="formField[{{$details->id}}][]" @else name="formField[{{$details->id}}]" @endif >
                            <option value="">{{$details->placeholder}}</option>
                            @foreach($details->option as $option)
                                  <option value="{{ $option->optionID}}">{{$option->dropdown_name}}</option>
                            @endforeach
                          </select>
              ...
              <div class="d-flex">
                    @foreach($details->option as $option)
                          ...
                          <label for="{{ $option->fieldcode }}id-formField[{{$details->id}}]">{{$option->dropdown_name}}</label>
                            <input class="custom-control-input ml-auto" id="{{ $option->fieldcode }}id-formField[{{$details->id}}]" type="radio" name="formField[{{$details->id}}]" value="{{ $option->fieldcode }}">
                          </div>
                        </div>
                   @endforeach
                      </div>
                      @elseif($details->fieldType=='file')
                <input type="file" class="form-control"  id="task_{{$details->code}}" name="file_formField">
                  <input type="hidden" name="formField[{{$details->id}}]" class="documents">
              @else
              <input @if($details->is_requried) required="requried" @endif class="form-control" type="text" id="task_{{$details->code}}" placeholder="{{$details->placeholder}}" name="formField[{{$details->id}}]" autocomplete="off" />
              @endif 
        ...
        <div class="tab wizard-tab second-page-tab">
          <div class="task-title">
            <h3>{{__('task_creation.Taskcreation_steptwo.Title')}}</h3>
          </div>
          <div class="create-task-form">
            ...
            <div class="d-flex">
              ...
              <input type="text" class="form-control" class="task_tags" name="task_tags"/>
              </div>

              <div  class="task__tags">
              <label>FTP Tag</label>
              <input type="text" class="form-control" class="ftp_tags" name="ftp_tags"/>
              </div>
            </div>  
            ...
            <div class="row document-items">
              <div class="col-sm-12 col-md-7">
                <div class="addi-attach">
                  <h3>{{__('task_creation.Taskcreation_steptwo.Additional Attachments')}}</h3>
                  <div class="d-flex">
                    <div class="custom-radio-box ">
                      <div class="custom-control custom-radio d-flex">
            ...
            <div class="mis-content">
              ...
              <div class="row">
              <div class="col-md-3 pr-0"><h3>{{__('task_creation.Taskcreation_steptwo.Commentary Requirement')}}</h3></div>
              <div class="col-md-3 pr-0"><h3>{{__('task_creation.Taskcreation_steptwo.MIS Requirement')}}</h3></div>
              <div class="col-md-3 pr-0"><h3>{{__('task_creation.Taskcreation_steptwo.Further Information Options')}}</h3></div>
              </div>
              <div class="row">
                ...
                <div class="col-md-3 pr-0"><button type="button" class="btn btn-primary btn-mis-select questionary-add" data-field-type="questionary" data-append-content="new-field" data-count="0">{{__('task_creation.Taskcreation_steptwo.Questionary')}} <span>+</span></button></div>
              </div>
              <input type="hidden" name="baseUrl" id="baseUrl" class="baseUrl" value="{{ url('/') }}">
              <div class="row mis-field-append-new-field mt-5">
                
              </div>
              @include('manager.task.include.addnewform')
            </div>
        </div>
        </div>
        @else
        <div class="tab wizard-tab second-page-tab">
		  ...
		  <div class="row">
              ...
              @elseif($details->fieldType=='long_text')
                <textarea @if($details->is_requried) required="requried" @endif class="form-control" placeholder="{{$details->placeholder}}" id="task_{{$details->code}}" name="formField[{{$details->id}}]"></textarea>
              @elseif($details->fieldType=='date')
                <input @if($details->is_requried) required="requried" @endif type="text" name="formField[{{$details->id}}]" class="form-control issuedatePicker" id="task_{{$details->code}}" data-value="task-{{$details->code}}" placeholder="{{$details->placeholder}}" autocomplete="off">
              @elseif(in_array($details->fieldType,array('dropdown_value','select2')))
              <select @if($details->is_requried) required="requried" @endif class="form-control select2" id="task_{{$details->code}}" id="task_{{$details->code}}" @if($details->fieldType=='select2') multiple="multiple" name="formField[{{$details->id}}][]" @else name="formField[{{$details->id}}]" @endif >
                            <option value="">{{$details->placeholder}}</option>
                            @foreach($details->option as $option)
                                  <option value="{{ $option->optionID}}">{{$option->dropdown_name}}</option>
                            @endforeach
              ...
              <div class="d-flex">
                    @foreach($details->option as $option)
                                <div class="custom-radio-box ">
                          <div class="custom-control custom-radio d-flex">
                          <label for="{{ $option->fieldcode }}id-formField{{$option->code}}">{{$option->dropdown_name}}</label>
                            <input class="custom-control-input ml-auto {{ $option->code }}" id="{{ $option->fieldcode }}id-formField{{$option->code}}" type="radio" name="formField[{{$details->id}}]" value="{{ $option->optionID }}" data-optionText={{$option->fieldcode}}>
                            </div>
                            </div>
                   @endforeach
                          </div>
        ...
        <div class="task-creation-footer">
          <div class="text-right">
            ...
            <button @if($formType=='task') type="button" id="submitBtn" @endif @if($formType=='issue') type="submit" style="display:none" id="issue-submitBtn" @endif @if($formType=='risk') type="submit" @endif  class="btn next-btn">{{__('task_creation.Taskcreation_steptwo.Submit')}}</button>
  ...
  <div class="modal fade" id="approvedModal" tabindex="-1" role="dialog" aria-labelledby="approvedModal" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-body">
          <h3>{{"This ".$formType." has been created"}}</h3>
